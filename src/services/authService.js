...
const login = async (data) => {
  ...
  try {
    const res = await fetch(api + `/${route}/login`, config)
      .then((res) => res.json())
    ...
    if (res._id) {
      ...
      return {
        errors: ["Erro de conexão com o servidor, tente novamente mais tarde."],
      };
    }
...
const verifyToken = async (token) => {
  ...
  try {
    const res = await fetch(api + `/${route}/token`, config)
      .then((res) => res.json())
      .catch((err) => err);
